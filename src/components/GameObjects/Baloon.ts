...
export default class Baloon extends Sprite implements Vehicle {
  ...
  isColliding(other: Sprite): boolean {
    return (
      Math.abs(other.position[0] - this.pos[0]) * 3 <
        other.width / 2 + this.width / 2 &&
      Math.abs(other.position[1] - this.pos[1]) * 1.5 <
        other.height / 2 + this.height / 2
    );
  }
  ...
  update(timeElapsed: number) {
    ...
    if (this._shouldMove[1]) {
      if (Math.abs(Level.map[this._index][this._side ^ 1] - this.pos[0]) < 6)
        return;
      if (
        Level.islands[this._index] &&
        Math.abs(Level.islands[this._index][this._side] - this.pos[0]) < 6
      )
        return;
